...
대화 답변 경로의 `AgentRuntime` 포트 구현 — owner OAuth 멀티-LLM(claude·codex·gemini 등)을 *기존 `AgentRuntime` 포트의 공급자별 어댑터*로 추상한 것(ADR 0027·Phase 9 — `StubRuntime`·`ClaudeCodeRuntime`과 *같은 포트*, 포트 무변경). **owner OAuth 인프로세스 스트리밍**이 핵심 — owner 워커가 owner OAuth 구독 토큰(API 키 아님)으로 공급자 API를 *인프로세스에서 직접 부르고* 토큰을 스트리밍한다(`claude -p` 프로세스 스폰·하네스 오버헤드 회피 = *속도 근거*, opencode/Hermes 방식). 자격증명 owner측이라 **중앙은 모델 토큰을 0개 보관**(ADR 0010 "중앙 키 0" 보존·강화). 구현: 공급자별 어댑터 `ClaudeApiRuntime`(첫 공급자·권장 Claude=Anthropic API+OAuth 구독·스트리밍 먼저 검증) → 후속 `CodexApiRuntime`·`GeminiApiRuntime`(*같은 포트·다른 transport*·`NotImplementedError` 자리, `HttpOidcProvider`·`SlackChannel` 정신). 요청/응답 매핑·스트리밍 토막 조립은 **순수 함수**(`build_provider_request`·`assemble_stream`·`map_response_to_answer` — SDK/IO 0·`serialize_reply`·`assemble_context`와 같은 투영 경계·`Answer` 계약 보존·노출 불변식)로 격리(`build_provider_request`는 `ProviderRequest(model·system·messages)`를 빌드 — `system`=카드 페르소나 투영[team·owner·summary·domains·can/cannot·sources]·`messages`=옵셔널 맥락+질문, `ClaudeCodeRuntime._build_persona_prompt`와 동형이되 Anthropic API system top-level이라 별 필드로), 답 생성에 **주입 transport**(`ProviderTransport` — `ClaudeRunner`가 `_run_claude_headless` 주입받는 정신)를 써 Stub 주입으로 결정론(요청/응답 매핑·스트리밍 조립 *결정 로직*만 게이트 내). **대화 답변 경로만 교체** — 분류기(`LlmClassifier`)·배치·골든셋 eval의 `claude -p`는 *잔존 가능*(명시 구분). **공급자 중립** — 어떤 공급자도 1급 아님(`NotificationChannel` 채널 중립 정신·한 공급자부터 증분·다중 공급자 동시 지원은 후속 명시 연기). 실 OAuth 흐름(owner CLI 위임 vs 직접 OAuth — 권장 보류·T9.6)·실 공급자 API 스트리밍·새 의존성(SDK/HTTP) 판단은 **게이트 밖 수동**(T9.6·`SubprocessGitGateway`·`HttpOidcProvider`가 실 본체를 게이트 밖으로 미룬 정신). **owner 워커 ↔ 중앙 WS 전송이 *기본 대화 경로*로 재부상**(ADR 0011·0012가 0017에서 "사설 데이터 옵션"으로 강등됐던 것을 0027이 1급으로 — owner 워커가 공급자 어댑터를 쥐고 돌아 중앙에 회신·ADR 0026 토큰 admission이 그 연결 검증). **불변식 영향 없음** — 미아 없음(런타임 교체가 종착 안 바꿈·owner 워커 부재는 백업[0012]→Manager[0014] 폴백)·Authority 중앙(런타임은 답 생성이지 권한 선언 아님·멀티 공급자는 모델 선택이지 권한 아님)·노출 불변식(`Answer` 계약 보존·맥락은 그 사용자 스레드만)·중앙 키 0(보존·강화). (ADR 0027·0010·0017·0011·0012)
