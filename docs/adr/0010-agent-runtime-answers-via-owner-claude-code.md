# Agent Runtime의 답변 주체는 각 Owner의 Claude Code다 — 중앙 API 키 LLM이 아니라 (→ "실행/분산" 부분 ADR 0017로 재정의)
...
> ⚠️ **부분 재정의 — ADR 0017 (2026-06-21).** 아래 본문의 "답변 주체 = owner Claude Code(*실행*)"와 "최종(T6.3) 분산 실행"은 *기본 경로에서 제외*돼 사설 데이터 커넥터 옵션(B)으로 이동했다 — owner는 *관리 주체(거버넌스)*이고 답 *실행*은 중앙 `claude -p`가 owner 통제 OKF 최신을 읽어 만든다. **여전히 유효한 건 "중앙 API 키 LLM RAG 회피 · `claude -p` 로컬 인증"뿐.** 현재 아키텍처는 **ADR 0017**. 아래 논증은 *그때의 추론 기록*으로 보존한다(역사 — 사설 데이터 케이스에서 분산을 부활할 때 근거).
