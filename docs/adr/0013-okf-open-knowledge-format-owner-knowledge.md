...
### 구현 분담(완료, mcp-runtime-engineer, 2026-06-21 — T6.7)

- **`ClaudeCodeRuntime` OKF cwd 소비** — `runtime.py` `_run_claude_headless(prompt, /, *, cwd=None, timeout=...)`: `cwd` 주어지면 그 디렉터리+`--allowedTools "Read,Glob,Grep"`(상수 `OKF_ALLOWED_TOOLS`), `cwd=None`이면 기존 tempfile 동작(도구 없음·하위호환). `_build_persona_prompt`에 "cwd OKF 먼저 읽고 근거로 답, 없으면 모른다고" 지시. `runner`는 `ClaudeRunner` Protocol(`__call__(prompt, /, *, cwd=None) -> str`)로 주입 결정론 보존.
- **번들 경로 규약** — `ClaudeCodeRuntime(runner, okf_root=None)` + `bundle_dir(card)`가 `okf_root/{agent_id}`를 *존재 시에만* cwd로 반환(없으면 None→tempfile 폴백). `knowledge_sources`(레이블)는 경로로 쓰지 않고 `agent_id`가 규약으로 디렉터리를 진다(카드 스키마 무변경·`Answer.sources` 보존). **`okf_root=None`(기본)이면 번들 해석 안 함** — 중앙 무지식·하위호환(암묵 cwd-상대로 owner 지식을 추정하지 않고 owner 환경이 명시 주입해야 소비). `answer`는 번들 있으면 `runner(prompt, cwd=...)`, 없으면 `runner(prompt)`(cwd 미전달 — 옛 1-인자 runner 런타임 호환).
- **샘플 OKF 번들** — `okf/cs_ops/`(index·refund-policy[20일·단순변심 45,000원 계산예시]·compensation) + `okf/contract_ops/`(index·standard-terms). 마크다운+YAML 프론트매터·type 자유(index/policy).
- **결정론 테스트** — `tests/test_okf_runtime.py`(11): 번들 있으면 cwd 그 경로로 전달 / `bundle_dir` 규약 / 레이블 아닌 agent_id 경로 / 프롬프트 OKF 지시 / 번들 없으면 cwd 미전달·옛 1-인자 runner 동작 / sources 보존 / subprocess `--allowedTools` 플래그·cwd None tempfile·비정상종료. 실 claude·실 파일 탐색은 수동/eval(게이트 밖). **게이트 324 passed**(기존 313 회귀 0), pyright strict 0, ruff 0.
- **demo/worker 와이어링** — `demo.py` `DEMO_OKF_ROOT`(repo `okf/` 절대경로·cwd 무관)를 `build_demo` 기본 런타임·`worker.py main()` `ClaudeCodeRuntime(okf_root=...)`에 주입(T6.3 분산·T6.6 백업 cwd 정합).
- **실 claude 시연 입증**(PoC 코드화 확인) — `okf/cs_ops` 번들 cwd로 "20일·단순변심·10만원 환불?" → "**45,000원**" + 계산근거 구체 답 / 번들 없는 cwd(okf_root=None)는 "문서 없어 모른다" 공허. ADR 본문 PoC(결정 1·3) 그대로 재현.
