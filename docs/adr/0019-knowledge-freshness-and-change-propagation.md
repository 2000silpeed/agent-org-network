...
상태: accepted (2026-06-22) · **구현 완료(tdd-engineer red→green 슬라이스 1~7 + adversarial 리뷰 수정 2건 — 707 passed/pyright 0/ruff 0)** · **T8.4(a) 정밀화(2026-06-24): 결정 7 — `ReevalSubject` sealed sum 분리(open question 해소)** · **ADR 0017 결정 3②·6의 본체**("정책이 바뀌면 그 정책에 기댄 *과거 Precedent·답*을 자동 재검토/무효화 플래그" — ADR 0017:45·79가 "변경 전파의 정확한 알고리즘 미해결"로 남긴 것을 이 ADR이 닫는다) · **ADR 0018 결정 6의 구체화**(커밋 SHA ↔ 신선도, "OKF 커밋 = 변경 이벤트 소스"의 자리를 본체로 채움) · ADR 0008(ConflictCase 처리함 패턴)·ADR 0012 결정 7(BackupReview 복귀 검토 루프)·ADR 0014(Manager 큐 수렴)와 정합
