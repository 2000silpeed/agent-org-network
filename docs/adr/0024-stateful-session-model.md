...
- **`answered_by`(내부 agent_id)는 transcript에 감사·라벨로 보존하되 *프롬프트 맥락엔 투영하지 않는다*.** `assemble_context`는 User/Assistant 발화 턴만 조립하고 내부 담당 식별자를 프롬프트에 싣지 않는다(노출 표면 최소화 — 어느 내부 담당이 답했나는 운영 면·감사의 정보지 다음 LLM 호출 맥락에 들어갈 값이 아니다). 메시지당 담당 1명이라 그 세션 담당이 곧 그 사용자의 대화 상대이므로 라벨 없이도 맥락이 온전하다(fan-out 시 라벨 규율은 후속).
