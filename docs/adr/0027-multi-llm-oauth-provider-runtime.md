...
상태: accepted (2026-06-26) · **후속: ADR 0031(SSE 토큰 스트리밍 seam·이벤트 계약·audit-once)** — 이 ADR이 "실 스트리밍(T9.6)"을 게이트 밖 후속으로만 명시하고 *seam을 비워둔* 것을 0031이 채운다(supersede 아님·refine — 코어 포트 무변경·옵셔널 `StreamingRuntime` 능력·`AnswerChunk`/`AskEvent` 계약). · **Phase 9의 ADR-D — 0010 supersede·0017 결정 2 재정의의 본체** · **ADR 0010 supersede**(0010이 0017로 한 번 재정의된 것을 *다시* 재정의 — 헤더에 포인터) · **ADR 0017 결정 2 재정의**(owner측 실행 부활 — 결정 2에 인라인 주석) · ADR 0011·0012 WS 전송을 *기본 대화 경로*로 재부상(0017이 "사설 데이터 옵션"으로 강등한 것을 1급으로) · ADR 0007(AgentRuntime 포트)·ADR 0013(OKF 지식 출처)와 정합 · CONTEXT(Agent Runtime 절 재정의·신규 Provider Runtime 용어)·PRD §3·§5·§6·TRD §2·§4 갱신 · **ADR 0029(OKF 자동 저작)가 새 사용**: 이 ADR의 owner OAuth 멀티-LLM은 *답변* 경로였고, 0029는 *같은 owner OAuth 인프라를 OKF 저작*(raw→OKF 초안 추출)에 씀(자격증명 owner측·중앙 토큰 0 보존·강화·포트는 분리 `OkfAuthor` ≠ `AgentRuntime`). · **ADR 0030(owner측 저작 토폴로지)이 그 새 사용을 실체화·강화**(supersede 아님·확장): `LlmAuthor`(`OkfAuthor` 실 구현)가 *답변 런타임 `ClaudeApiRuntime`과 같은 인프라*(같은 owner 워커·같은 provider transport·같은 OAuth 세션·`worker.py`)로 저작 추출·**semantic-os import 0**(core 기본 어댑터). 공급자 중립(결정 11)의 저작판 — core는 `LlmAuthor`만·`SemanticOsAuthor`(RDF)는 owner 확장점이라 core RDF 의존 0.
...
- **확정**: owner의 **Anthropic 자격**(`ANTHROPIC_API_KEY` env 또는 `ant auth login` 공식 console OAuth 프로필)을 워커가 재사용한다. 워커의 `anthropic.Anthropic()`(인자 없이)가 그 자격을 **자동 해석**한다. 직접 PKCE 흐름은 **기각**(무거움·재발명). **⚠️ 실 시연 정정(2026-06-27·아래 결정 9 참조)**: 인자 없는 SDK는 *기존 Claude Code `/login` 구독 자격*은 *해석 못 한다*(다른 위치·밴 위험) — 그 부분은 기각하고 **Claude 구독 답은 `claude -p`(`ClaudeCodeRuntime`)** 가 공식 경로다. 상세는 아래 **결정 9**.
...
- **⚠️ 실 시연 정정(2026-06-27) — Claude "owner CLI 위임 자동 해석"은 부분 오류였다.** 실 시연으로 확인: 인자 없는 `anthropic.Anthropic()`는 `ANTHROPIC_API_KEY` env·`ant auth login`(공식 console OAuth) 프로필만 해석하고 **Claude Code `/login` 구독 자격(`~/.claude/.credentials.json`·`claudeAiOauth`)은 *해석 못 한다*** — codex의 `~/.codex/auth.json`과 *다른 위치·다른 메커니즘*. 게다가 캐시 토큰은 만료돼도 `claude -p`가 *인메모리로만* 갱신(파일 stale)하고, 구독 토큰을 Claude Code 밖에서 직접 API에 쓰는 건 **Anthropic ToS 위반·계정 정지 위험**(커뮤니티 경고). → **결정(사용자 2026-06-27): 공식 경로만.** Claude 인프로세스 SDK transport는 *`ANTHROPIC_API_KEY`/`ant` OAuth* owner용이고, **Claude 구독 답은 `claude -p`(`ClaudeCodeRuntime`)가 공식·robust 경로**(실 시연 답 생성 확인). 결정 2의 "기존 Claude Code 로그인 재사용"은 **기각** — Claude와 codex는 *비대칭*(codex만 자기 CLI 토큰 파일로 구독 인프로세스가 성립). 공급자 중립·코어 무의존(결정 11)·중앙 토큰 0은 그대로.
  - **✅ 깊은 검증(2026-06-27·throwaway 스크립트)** — 사용자 요청으로 "정책 재허용(reinstated with a catch)"을 끝까지 실측. live 자격은 `~/.claude/.credentials.json`이 아니라 **macOS Keychain `Claude Code-credentials`**(`security find-generic-password`)에 있고, `refreshToken`을 **`https://platform.claude.com/v1/oauth/token`**(client_id `9d1c250a-...` — Claude Code OAuth)로 갱신하면 fresh `access_token`(8h)이 나온다. 그걸 `anthropic.Anthropic(auth_token=…, default_headers={"anthropic-beta":"oauth-2025-04-20"})`로 쓰면 **인증은 통과**(401 아닌 429 = 수락). → *기술적으론 가능*. **그러나 viable하지 않다**: (1) **refresh 토큰이 단일 사용(rotation)** — 외부에서 한 번 갱신하면 Keychain의 토큰이 무효화돼 **Claude Code 자체 로그인이 깨진다**(실측으로 owner의 `claude -p`가 401로 죽음·`claude /login` 재인증 필요). Claude Code와 *공존 불가*. (2) Messages API가 **간헐 429/401**(구독 OAuth를 Claude Code 밖에서 쓰는 것에 대한 Anthropic anti-abuse — "the catch"). → **결론 불변: `claude -p`(`ClaudeCodeRuntime`)가 Claude 구독의 robust·sanctioned 경로**(자기 자격·rotation을 내부에서 안전 관리). 인프로세스 구독 OAuth는 *만들지 않는다*.
- **codex(OpenAI) 측 OAuth 위임 — bespoke(슬라이스 2·2026-06-27 구현·✅ 실 시연 검증 완료)** — anthropic의 "인자 없는 클라이언트 자동 해석"과 달리 codex는 *owner 기기 `~/.codex/auth.json`*(ChatGPT 구독 OAuth·codex CLI가 백그라운드 갱신·`CODEX_HOME` 또는 `~/.codex`)을 워커가 직접 읽어 OAuth `access_token`·`chatgpt_account_id`를 추출한다(방어적 파싱·JWT claim 폴백 — 정확한 키 구조는 demo 검증). openai SDK 클라이언트를 **ChatGPT 구독 base_url `https://chatgpt.com/backend-api/codex`**(API 키 경로 `api.openai.com` *아님*)로 만들고 **Responses API**(`instructions`=system·`input`=messages·SSE `response.output_text.delta` 델타)로 스트리밍한다. 헤더 `ChatGPT-Account-ID`·`User-Agent: codex_cli_rs/...`. 401→auth.json 재독→재시도 1회. 자격은 여전히 *owner측·중앙 토큰 0*. (`CodexOauthTransport`·`provider_transport_codex.py`. SDK가 ChatGPT 구독 경로/SSE 형식에 안 맞으면 raw httpx 폴백 — 게이트 밖이라 *실제로 통하는 쪽*을 demo로 정한다.)
