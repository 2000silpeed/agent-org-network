...
- **codex(OpenAI) 측 OAuth 위임 — bespoke(슬라이스 2·2026-06-27 구현·실 검증 대기)** — anthropic의 "인자 없는 클라이언트 자동 해석"과 달리 codex는 *owner 기기 `~/.codex/auth.json`*(ChatGPT 구독 OAuth·codex CLI가 백그라운드 갱신·`CODEX_HOME` 또는 `~/.codex`)을 워커가 직접 읽어 OAuth `access_token`·`chatgpt_account_id`를 추출한다(방어적 파싱·JWT claim 폴백 — 정확한 키 구조는 demo 검증). openai SDK 클라이언트를 **ChatGPT 구독 base_url `https://chatgpt.com/backend-api/codex`**(API 키 경로 `api.openai.com` *아님*)로 만들고 **Responses API**(`instructions`=system·`input`=messages·SSE `response.output_text.delta` 델타)로 스트리밍한다. 헤더 `ChatGPT-Account-ID`·`User-Agent: codex_cli_rs/...`. 401→auth.json 재독→재시도 1회. 자격은 여전히 *owner측·중앙 토큰 0*. (`CodexOauthTransport`·`provider_transport_codex.py`. SDK가 ChatGPT 구독 경로/SSE 형식에 안 맞으면 raw httpx 폴백 — 게이트 밖이라 *실제로 통하는 쪽*을 demo로 정한다.)
