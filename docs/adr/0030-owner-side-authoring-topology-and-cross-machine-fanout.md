...
- **크로스머신 변경 전파 사슬**:
  ...
  owner 커밋(로컬 git)
    ...
    → reindex 소비자(로컬·증분 재인덱싱·T11.5)    ← reindex_incrementally
...
텍스트 인제스트(TextIngestor·기존 T11.6)
  ...
  → 재인덱싱(reindex_incrementally·기존 T11.5)
...
- **reindex 소비자**: `OkfChangeEvent` 수신 → `reindex_incrementally`(T11.5 green) → 재인덱싱 산출 → 워커 `publish_frames`(ADR 0028 §14·`worker.py` green)로 `PublishIndex` 송신.
...
- **`OkfChangeEvent` 구성(중앙측)**: `accept_published_index`는 commit을 안 보므로 `new_sha`·`parent_sha`·`changed_paths`를 알 수 없다. reeval은 *agent_id 거친 매칭*(ADR 0019 결정 2 — agent_id만 영향 식별에 쓰고 changed_paths는 죽은 필드)이라, 중앙측 `OkfChangeEvent`는 `agent_id`(= index.agent_id)·`committed_at`(= 수용 시각 또는 index.generated_at)만 채우고 SHA류는 None/placeholder로 둔다(reeval이 그 필드를 안 쓰므로 무영향).
