...
**후속 가산(이번 미룸·포트 뒤·재작업 0)**: PDF/docx/위키 `Ingestor` 어댑터·`SemanticOsAuthor`(owner 확장점)·리치 UI·골든셋 eval 깊이(결정 4) · ~~T10.5 `EmbeddingAnnMatcher`·임베딩 모델 선택~~ → **해소(2026-07-02)**: 결정 5의 보류 전제("압박이 실제 있을 때")를 10-에이전트 실 자료 골든셋 실측으로 확인·`EmbeddingAnnMatcher` 구현·채택(docs/scale-eval-2026-07-02.md·ADR 0028 §7 현금화·모델=기존 `FastEmbedEmbedder`/e5-small 재사용). 실 RAG `ConfidenceAssessor`는 여전히 후속(contested 볼륨 압박 시·결정 5 잔여 — 실측이 그 압박[contested 45.8%]도 이미 정량화함).
...
- **중앙 토큰 0 / 비소유(보존·강화)**: 저작 LLM(owner OAuth·owner 워커)·raw·초안(owner측·중앙 WS 미도달)·OKF git(owner-로컬·중앙 클론 0)이 모두 owner측, 중앙은 published 목차만 받는다(§비소유 논거). reeval이 index 수용으로 트리거(중앙이 commit·git을 안 봄). ADR 0027 "중앙 키 0"을 저작 토폴로지에서도 보존·강화. **T11.7e E5 3각도 실증**: 실 WS 와이어 본문 0·`Concept` 스키마에 body 필드 부재·`server.py` anthropic SDK 미import(LLM 호출은 owner 워커 OAuth in-process) — 중앙 토큰 0이 실 소켓 관통에서도 성립.
...
- **발화 단일[머신별](보존·재해석)**: owner commit이 reindex만·중앙 index 수용이 reeval만 발화·크로스머신 이벤트 중복 0(결정 3). ADR 0019 "발화 단일 지점"이 머신별 단일로 진화하되 한 변경이 두 번 reeval되거나 누락되지 않는다. 단일 머신 배포에서도 WS 루프백으로 동일 구조. **T11.7e 실증**: owner 실 커밋이 reindex(재-publish)만·중앙 실 WS 수용이 reeval 양축(Answer·Precedent)만 발화 — 실 소켓 왕복에서 머신별 단일 분리를 실증.
...
> **T11.7e 관통(2026-07-01)으로 아래 4개 해소** — 얇은 수직 슬라이스를 게이트 내 배선 3건 + 게이트 밖 크로스 프로세스 관통(E3~E5·단일 머신 WS 루프백·실 소켓·실 직렬화·실 SHA·실 LLM owner OAuth)으로 한 번 관통해 되돌리기 어려운 토폴로지 결정들이 *실증*됐다. 관통 중 프로덕션 코드는 게이트 내 배선(E1·E2·minor-1)만 바꿨고 E3~E5는 기존 조립만(0 변경)·`ANTHROPIC_API_KEY` UNSET 유지. **과대주장 금지** — 단일 머신 루프백은 실 소켓·실 직렬화까지 실증하나 물리 2대 네트워크 지연 특성은 미측정.

- **[해소·2026-07-01] sink 실패 정책 = 흡수+경고 로그(재시도 큐 없음)** — owner commit 후 재-publish(`ReindexOnCommitListener.on_okf_committed`)의 `publish_sink` 예외를 fail-fast(커밋 경로 깸)도 at-least-once 재시도 큐도 아닌 **흡수+`logging.warning`**으로 확정(사용자 승인·T11.7e E2·게이트 내 잠금). 커밋은 이미 디스크 확정이라 전파 예외가 owner commit을 깨선 안 되고, 전파 회수는 **재연결 백스톱**에 위임한다 — `run_worker`가 register 직후 `publish_frames` 전량을 재송신하고 중앙 `accept_published_index`의 `generated_at` 멱등(더 새 것만·동률/역행 거부)이 중복을 흡수한다(별 재시도 큐 불요). 남은 것: 재연결 폭주 시 재송신 볼륨 임계는 실 배포 관측 후(아래 지연·중복 항목).
- **[해소·2026-07-01] 토폴로지 = 단일 머신 WS 루프백으로 관통 완료** — E3~E5가 인프로세스 store 공유를 넘어 **별도 프로세스 실 소켓 왕복**(`run_worker` 실 WS `PublishIndex` 송신 → 중앙 `/worker` accept)으로 전 사슬을 실증했다(E3 publish 전 `unowned`[0매칭→root escalation·미아 없음] → 후 `dispatched`[cs_ops Routed] 라우팅 전환). **여전히 후속(과대주장 금지)**: 물리 2대(별 머신) 실 네트워크 지연·재연결 reeval/재송신 폭주 임계는 실 배포 관측 후 확정 — 단일 머신 루프백은 발화 구조·직렬화까지 같으나 물리 네트워크 지연 특성은 미측정.
- **[해소·2026-07-01] owner-로컬 repo = 격리 git repo** — E3~E5가 owner_okf를 프로젝트 저장소와 **분리된 격리 git repo**에 `SubprocessGitGateway` 실 커밋으로 굴려(E3 `ca04dae`→`b7e4985`·E5 `df1ffc3`→`026d243`·실 SHA) *프로젝트 저장소 오염 0*을 실증했다(중앙 모노repo 아님·결정 3 "owner-로컬 repo" 실체). 남은 것: owner별 repo 초기화 자동화·GitHub 선택 백업 동기화·한 owner 다중 카드 repo 구조는 운영면 후속.
- **[해소·2026-07-01] 공급자 transport = claude-api(claude-sonnet-5·owner OAuth)** — 첫 demo 공급자 transport는 `AON_PROVIDER=claude-api`(owner `ant` OAuth·`claude-sonnet-5`)로 확정·실증(E5 실 `LlmAuthor` 추출 + `/ask` 워커 실 `ClaudeApiRuntime` 답변). 공급자별 OAuth 위임 비대칭(아래 항목)은 ADR 0027 결정 9를 그대로 따른다(이 ADR이 재정의 안 함) — claude-api는 owner OAuth 인프로세스 SDK로 성립함을 실증. register 인증은 자리만(`_authenticate`가 `bool(owner_id)`만 검사·실 토큰 검증 미구현·ADR 0026 후속).

- **`LlmAuthor` 추출 품질** — 개념 분할 입도·core_question 정확도·edges 과/소 연결(ADR 0029 OQ ①②③ 연장). 골든셋 eval(ADR 0003)·게이트 밖. T10.5(b) distill을 흡수하므로 그 품질 책임 지점이 `LlmAuthor`로 합쳐진다. T11.7e E5 정직한 한계: 실 LLM이 교환정책 입력에서 '교환' over-claim 3건 drop·'환불' 1건 수용 — 수용 1건은 입력 선택 이슈이지 파이프라인 결함 아님(over-claim 필터·권한 중앙 강제는 정확 작동).
- **크로스머신 전파 지연·중복** — owner commit ↔ 중앙 index 수용 사이 지연(워커 부재·WS 끊김)에서 staleness가 더 새 것 멱등 흡수(ADR 0028 §14 결정 C)로 중복을 닫으나, 실 크로스머신 ordering·재연결 reeval/재송신 폭주 임계는 실 인프라(물리 2대) 관측 후(게이트 밖). T11.7e 단일 머신 루프백은 실 소켓·실 직렬화가 인프로세스 store 공유와 동치임(더 새 `generated_at`만 수용·멱등)을 실증했으나 물리 네트워크 지연은 미측정.
- **reeval 인덱스-수용 훅의 과검출** — index 수용마다 그 agent_id 과거 판례를 reeval 큐에 올리면(agent_id 거친 매칭·ADR 0019 결정 2), 잦은 재배포가 reeval 노이즈를 낸다. ADR 0019 "놓침 0 > 과검출 0"·owner가 처리함에서 흡수. 정밀화(changed_paths 활용)는 후속. T11.7e E4가 양축(Answer·Precedent) reeval 실 WS 발화를 실증(owner 세션 `/inbox/reeval` 실 적재).
