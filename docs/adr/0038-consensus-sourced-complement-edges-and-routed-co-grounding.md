...
- **슬라이스**: A(방출·저장)·B(selector)·C(AskOrg Routed arm 배선)는 게이트 내 결정론(tdd-engineer), D(프로덕션 EdgeStore 공유 배선·실 LLM Routed D1→D2 재현)는 mcp-runtime-engineer·일부 게이트 밖 수동.
