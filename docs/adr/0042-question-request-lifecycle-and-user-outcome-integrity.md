...
- **⑧ 답 수신의 종착 매핑 — S5.4는 `Answered` 하나만 durable하게 닫는다.** `AwaitingAnswer`의 네 종착 중 **`AnsweredRequest`**만 S5.4가 소유한다(lease release + ticket `completed` + S5 receipt + `complete_in_transaction`을 한 transaction). **`AwaitingApproval`**은 `ApprovalBoundary`가 ApprovalItem 생성과 함께 소유하므로 S5가 그 전이를 만들지 않는다 — 승인이 필요한 후보는 S5.4가 **Conflict 계열이 아닌 전용 typed 거부**(`DurableAnswerApprovalRequired` — Conflict의 형제이지 하위가 아니다)로 fail-closed하고, 기존 in-process 승인 경로가 그대로 종착을 책임진다(회귀 0·ticket은 `pending`으로 남고 S4.6은 이를 tolerant로 통과). Conflict의 하위로 두지 않는 이유는 **승인 필요가 실패가 아니라 “다른 경로로 가라”는 경로 신호**이기 때문이다 — 하위로 두면 `except ...Conflict: skip/retry`를 하는 호출자가 그 신호를 조용히 삼킨다. **`AwaitingManager`**는 S5.6이다. **`FailedRequest`는 S5의 어떤 경로도 쓰지 않는다** — §2의 “일시 장애는 곧바로 Failed로 닫지 않는다”를 채널 영구 실패까지 확장한다. 워커가 영구 미연결이거나 전달이 반복 실패해도 결과는 Failed가 아니라 **사람에게 닿는 escalation**이다. Failed는 도메인적으로 복구 불가이면서 사람 처분으로도 진행할 수 없을 때만 쓰며, S5 범위에는 그런 경우가 없다.
- **⑨ SLA와 lease는 다른 시계다.** lease 만료는 “이 전달을 맡은 중앙 인스턴스가 멈췄다 → 재전달”이고, `AwaitingAnswer.handling.due_at`(§2의 도메인 SLA)은 “이 실행 시도가 너무 오래 걸렸다 → 사람에게”다. S5.5 timeout scan의 판별자는 **후자**이며, 전자는 S5.2 reclaim이 처리한다. scan은 read-only(write·cursor·scheduler 0)이고 후보 조건은 `ticket.status='pending'` ∧ Request가 그 ticket에 결박된 `AwaitingAnswer` ∧ `revision == ticket.awaiting_revision + 1` ∧ `handling.due_at <= now`다. **이 보고는 작업 목록(index)이지 write 근거(evidence)가 아니다** — 소비자는 자기 transaction 안에서 자기 `clock()`으로 SLA를 다시 판정해야 한다(⑰). scan이 `now`를 파라미터로 받는 것은 그것이 write를 하지 않기 때문이며, **그 안전성은 이 모듈 안에서만 참이고 소비자로 전이되지 않는다**.
...
- **⑫ `SQLITE_BUSY`는 Conflict가 아니라 별 타입의 Unavailable이다 (2026-07-25·S5.2 독립 리뷰 P2-1 판정).** 다른 connection과 lock 경합이 나면 `sqlite3.OperationalError`가 무타입으로 누수한다. 이 taxonomy는 이미 선례가 답을 갖고 있다 — `answer_finalization_sqlite._is_storage_unavailable`이 extended result code의 primary byte로 `SQLITE_BUSY`·`SQLITE_LOCKED`를 **“재시도 가능한 저장소 실패”**(`SqliteCompletionStorageUnavailableError`)로 분류한다. 같은 파일·같은 lock 도메인이므로 S5가 다른 의미론을 만들면 한 DB에 두 taxonomy가 생긴다. 따라서 **BUSY를 Conflict로 흡수하지 않는다** — Conflict는 “경쟁자가 이겼다”는 *도메인 사실을 알았다*는 뜻인데 BUSY는 transaction이 상태를 읽지도 못한 *모름*이고, 둘을 섞으면 S5.7의 “정확히 1승” 단언이 CAS 패자와 lock 패자를 구분하지 못해 검증 불가가 된다. 그렇다고 일반 Unavailable로만 승격해도 안 된다 — 그러면 배치 runner가 살아남기 위해 Unavailable 전체를 잡아야 하고, **손상된 capability·parent 부재까지 삼킨 채 남은 ticket을 조용히 헛돌게** 된다. 판정은 **`DurableDispatchLeaseBusy(DurableDispatchLeaseUnavailable)` 신설**이다: 의미론은 Unavailable(모름)로 정확히 두고, 처리는 명시적으로 갈라 준다(기존 `except ... Unavailable` 호출자는 그대로 동작하는 additive 변경). 판별은 문자열 매칭이 아니라 `(error.sqlite_errorcode & 0xFF) in {SQLITE_BUSY, SQLITE_LOCKED}`이며, **BUSY/LOCKED만** Busy로 좁힌다 — `IOERR`·`FULL`·`READONLY`·`CANTOPEN`은 재시도로 풀리지 않으므로 일반 Unavailable로 남아 runner를 멈춰야 한다(Completion은 호출자가 사람 요청 1건이라 단일 버킷으로 충분했고, 배치 runner인 S5는 이 구분에 실익이 있다 — 모순이 아니라 정밀화다).
  - **대기 시간 설정 위치 — S5는 설정하지 않는다.** lease UoW는 Completion이 소유한 connection을 `durable_transaction()`으로 빌려 쓴다. 거기에 `PRAGMA busy_timeout`을 걸면 다른 component가 공유하는 자원의 성질을 S5가 바꾸는 소유권 누수다. 대기 시간은 이미 `SqliteQuestionCompletionUnitOfWork(timeout=...)`(기본 5.0초) 생성자 인자로 표현돼 있고, 조정이 필요하면 그 composition 지점이 유일한 손잡이다. 자기 connection을 여는 S5.5 scanner만 자기 `timeout`을 정한다. **WAL 전환은 S6 이월** — journal mode는 DB 전역 속성이고 Completion migration 소유라, 지금 바꾸면 기존 전 component의 동시성 성질이 함께 바뀐다.
  - **각 층의 처리 계약.** **S5.3 runner:** `Conflict → skipped`(이 ticket은 지금 대상이 아니다·정상), `Busy → contended`(락 압력·다음 run 재시도), **비-Busy Unavailable은 잡지 않고 run 전체를 중단**한다. `skipped`와 `contended`를 한 카운터로 합치지 않는다 — 전자는 정상 관찰, 후자는 `timeout`·`batch_limit`을 조정할 운영 신호이며 합치면 그 근거가 사라진다. TX-B(전송 뒤 기록)의 Busy는 시도 행 미기록·lease leased 유지이므로 **crash point ②와 관측 결과가 같다**(만료 뒤 `epoch+1` 재전달) — 새 경로가 아니라 이미 선언한 at-least-once 봉투 안이다. **S5.4:** 워커 제출 응답 경로이므로 흡수하지 않고 그대로 올린다(호출자인 전송 계층이 재시도/오류 응답을 정한다 — 워커에게 “지금 붐빔”이 전달돼야 한다). **S5.5 scanner:** capability-우선 fail-closed 규율대로 `capable=False`·`candidates=()`.
- **⑬ ⑤의 “호출자가 만료를 주장할 수 없다”에는 명시된 예외가 하나 있다 (2026-07-25·S5.2 독립 리뷰 P2-5 판정).** 금지되는 것은 **판정 결과(만료 여부)를 인자로 넘기는 것**이고, public `claim`/`renew`/`release`에는 시각 인자가 없다(판정은 언제나 UoW가 저장된 `expires_at`과 자기 시각을 비교해서 한다). 허용되는 예외는 **이미 열린 write를 소유한 형제 컴포넌트가 그 transaction의 단일 시각을 나르는 `*_in_transaction(now=)` seam**이다 — 한 transaction 안에서 시도 행 `created_at`과 lease `expires_at`이 다른 시각을 보면 안 되기 때문에 필요하다. 안전 논거는 세 겹이고, 셋째가 본질이다.
  1. **연장 봉쇄** — renew CAS의 `expires_at > now`가 이미 만료된 lease의 부활을 막는다.
  2. **하한 봉쇄** — `now >= acquired_at` CAS 가드가 `expires_at >= acquired_at` 행 불변식 위반을 막는다.
  3. **단축은 안전 방향이다 — lease는 정확성 fence가 아니라 중복 억제 최적화다.** 시각이 과거로 밀려 lease가 짧아지면 재전달이 빨라질 뿐이고 그건 ⑥이 이미 선언한 at-least-once 봉투 안이다. “답은 한 번만 종결된다”는 정확성은 lease가 아니라 **S5.4의 ticket status CAS와 Completion receipt**가 지킨다. 그래서 이 seam이 시각을 나르는 것은 어떤 불변식도 위태롭게 하지 않는다.
  - **`DispatchLease` 값 객체 계약(P2-2 판정).** `DispatchLease`는 **UoW가 발급한 값만 유효**하며, `renew`·`release`·`*_in_transaction`은 CAS `WHERE`에 `org_id`·`request_id`를 포함해 저장 행과 대조한다. 이 둘을 무검증 echo하면 위조·stale lease가 S5.3 시도 행에 **타 조직 lineage**를 남길 수 있고(S5.1 row 검증이 나중에 손상으로 잡지만 그때는 이미 기록된 뒤다), `WHERE` 항 두 개의 비용은 사실상 0이다. 값 객체에서 두 필드를 빼고 매번 ticket을 재조회하는 대안은 TX-B에 SELECT를 더할 뿐 같은 성질을 주므로 채택하지 않는다.

- **⑭ 배치 runner의 실패 분류 기준은 “외부 효과가 있었는가”와 “어떤 손잡이를 돌려야 하는가”다 (2026-07-25·S5.3 독립 리뷰 P2-2·P2-7 판정).**
  - **TX-B에서 lease를 뺏긴 경우 — 흡수하고 계속한다.** 전송 중 TTL이 만료돼 다른 인스턴스가 reclaim하면 TX-B의 lease CAS가 `Conflict`를 올린다. 이것을 배치 중단 사유로 삼지 않는다. 근거 둘: ⑬대로 **lease는 정확성 fence가 아니므로** 탈취가 어떤 불변식도 깨지 않고(시도 행 롤백·ticket `pending`·상대가 소유), 관측 결과는 ⑥이 이미 선언한 **crash point ②(전송했으나 미기록)와 동일**해 새 상태가 아니다. 배치 중단을 기각하는 이유는 그것이 “이 인스턴스가 뒤처졌다”는 사실과 **무관한 2차 피해**(남은 ticket 미처리·report 소실)를 만들기 때문이다 — 뒤처졌다는 신호는 아래 카운터가 정확히, 손실 없이 전달한다. 단 TX-A의 `Conflict`와 **같은 카운터로 합치지 않는다**: TX-A는 “가져오지 못했다”(외부 효과 0)이고 TX-B는 “보냈는데 기록하지 못했다”(외부 효과 1)로, 사실이 다르다.
  - **카운터를 나누는 기준 = 돌려야 할 손잡이가 다르면 다른 카운터**(⑫에서 `contended`를 `skipped`와 분리한 것과 같은 원리). **`skipped`** = 이 ticket은 지금 대상이 아니다(손잡이 없음·정상 관찰). **`contended`** = lock 압력(손잡이: Completion `timeout`·`batch_limit`). **`preempted`** = TTL보다 전송이 느려 lease를 뺏겼다(손잡이: `lease_ttl`·채널 지연). 세 값을 합치면 각각의 조정 근거가 사라진다. `preempted`는 시도 행이 없으므로 `delivered`/`undeliverable` 어디에도 세지 않는다.
  - **러너 전용 오류 계열을 신설한다.** 러너의 자기 결함(잘못된 `batch_limit`·tz-naive clock·claim된 lease의 ticket이 사라진 불가능 상태)을 lease component의 `...LeaseUnavailable`로 올리면, 호출자가 **“러너 설정이 틀렸다”와 “lease capability가 손상됐다”를 구분할 수 없다** — ⑫가 “호출자가 실패 종류를 구분해야 abort/계속을 정할 수 있다”고 정한 원리를 한 층 밖으로 적용한다. 계열은 lease 쪽과 같은 모양(`Error` / `Unavailable` / `Busy <: Unavailable`)으로 두고, **lease 계열 예외는 wrap하지 않고 그대로 통과시킨다**(그 구분이 신설 목적이다). S5.4·S5.5는 이미 자기 계열을 갖고 있어 S5.3만 비대칭이었다.
  - **read 지점의 BUSY.** 후보 조회의 BUSY는 후보를 하나도 못 읽은 것이므로 **run 전체를 typed Busy로 되돌린다**(다음 tick 재시도). claim 이후 per-ticket read의 BUSY는 아직 외부 효과가 0이므로 **그 ticket만 `contended`로 흡수**하고 다음으로 넘어간다 — 그 ticket은 lease TTL이 만료될 때까지 막히지만(crash point ①과 동일) 그게 정확한 대가다. ⑫의 3분기 계약을 read 지점까지 확장한 것이며 새 결정이 아니다.

- **⑮ 승인 축은 “언제 알 수 있는가”로 나뉘어 두 컴포넌트가 나눠 갖는다 (2026-07-25·S5.4 독립 리뷰 P2-1 판정).** ⑧의 승인 거부는 세 축 중 둘만 S5.4가 본다. 이건 누락이 아니라 **탐지 시점에 따른 필연적 분담**이다.
  - **route/mode 축** — `route.requires_approval`·`candidate.mode == "draft_only"`는 command-local 값과 저장된 route만으로 **어떤 write보다 먼저** 알 수 있다. S5.4가 소유하고 `DurableAnswerApprovalRequired`로 거부한다(write 0).
  - **policy 축** — “gate 이후 중앙 정책이 바뀌어 지금은 승인이 필요하다”는 **중앙 ApprovalPolicy를 실제로 평가해야만** 알 수 있다. Completion의 `_validate_no_approval`이 commit 직전 한 transaction 안에서 평가하고 `CompletionEvidenceError`로 막는다(원자 롤백 → 역시 write 0).
  - **S5.4가 policy 축을 번역하지 않는 이유.** (i) `CompletionEvidenceError`는 “정책이 승인을 요구함”·“정책 버전 drift”·“정책 조회 불가”를 한 타입에 담으므로, 타입만 보고 번역하면 **일시 장애를 “승인 필요”로 오역**한다. (ii) 구분하려면 S5.4가 ApprovalPolicy를 직접 평가해야 하는데 그건 ⑧이 금지한 “승인 판단 소유”다. (iii) 미리 평가해도 그 뒤 commit까지 정책이 또 바뀔 수 있어, transaction 안의 검사만이 권위이고 사전 검사는 장식이다.
  - **⑫·⑭의 원리와 충돌하지 않는다.** 그 원리는 “호출자가 **조치를 정할 수 있어야** 한다”이지 “타입이 최대한 잘게 갈려야 한다”가 아니다. 실제 조치를 보면 route/mode 축과 policy 축은 **둘 다 ‘ApprovalBoundary로 재-gate하라’** 하나로 수렴하고, 저장소 손상(`...IngestionUnavailable`·`CorruptSqliteCompletionError`)·일시 장애(`...Busy`)와는 **이미 타입으로 구분된다**. 즉 호출자가 못 고르는 선택지가 없다.
  - **이 분담이 성립하는 조건 — 불변식으로 못박는다.** S5.4는 `AnswerFinalizationError` 계열(특히 `CompletionEvidenceError`)을 **wrap하지 않고 그대로 통과시켜야 한다.** 자기 계열로 감싸는 순간 policy 축이 저장소 오류와 섞여 호출자의 조치 선택이 실제로 무너진다. 이는 ⑫·⑭의 “남의 계열은 wrap 없이 통과”와 같은 규율이다.
- **⑯ 남의 컴포넌트 행을 직접 쓰면 그 컴포넌트의 행 불변식을 CAS 조건으로 옮겨 적어야 한다 (2026-07-25·S5.4 P1 재발 판정 — S5.6 선행 규율).** S5.2는 자기 테이블이라 seam 시각 하한(`acquired_at`)을 CAS에 넣었지만(⑬-2), S5.4는 **남의 테이블**(`durable_dispatch_leases`)을 직접 UPDATE하면서 같은 가드를 빠뜨렸다. 이 실수의 대가가 유독 큰 이유는 S5.1 같은 validate-only 컴포넌트의 `_validate_rows`가 **모든 writer가 매 transaction마다 재실행하는 전역 검사**이고 **보정 경로가 설계상 없기** 때문이다 — 잘못된 행 하나가 그 컴포넌트를 **전 조직·전 연산에 대해 영구 Unavailable로 만든다**. 지역 버그가 아니라 회복 불가능한 전역 정지다. 따라서:
  1. **먼저 seam을 찾는다.** 소유 컴포넌트에 위임할 메서드가 있으면 직접 SQL을 쓰지 않는다. S5.4가 직접 쓴 건 lease seam이 `(lease_epoch, holder_ref)` fence를 요구하는데 답 경로는 lease로 fence하지 않기 때문(⑧)이다 — **seam 부재가 직접 write를 부른다**. 그런 경우 “왜 seam이 없나”를 먼저 답하고, 답이 “그 fence가 이 경로엔 부적절해서”라면 아래 2를 지킨다.
  2. **소유 컴포넌트의 row 검증 규칙을 전수 대조하고, 그중 이 write가 깨뜨릴 수 있는 것을 전부 CAS `WHERE`로 옮긴다.** 쓰기 전 `if` 검사로 대체하지 않는다 — 코드 흐름의 조건은 나중에 분기가 추가되면 우회되지만 CAS 조건은 우회할 수 없다.
  3. **바꾸지 않는 컬럼은 손대지 않는다.** enum 값 하나만 전방 전이시키는 write(예: ticket `status`)는 다른 불변식을 건드릴 수 없어 안전하지만, 그 판단은 1·2를 거친 **결론**이어야지 생략의 근거가 아니다.
  - **S5.6 적용.** S5.6은 자기 escalation component 외에 S4.1 `durable_linked_work_tickets.status`(`pending → escalated`)를 직접 쓴다(②의 교차 소유권 계약). 착수 시 S4.1 `_validate_rows`의 ticket 규칙 전수와 대조해 1~3을 명시적으로 통과시킨다.

- **⑰ S5.6 escalation의 시간 안전 의무 — 스캔 결과는 작업 목록이고, SLA 판정은 write transaction이 다시 한다 (2026-07-26·S5.5 독립 리뷰 P1-1 판정).** ⑨의 “scan은 write가 없으니 호출자 `now`를 받아도 안전하다”는 **그 모듈 안에서만 참이며 소비자로 전이되지 않는다.** 전이를 가정하면 실제 피해 경로가 열린다: 미래 `now`로 스캔 → 아직 SLA 안인 ticket이 후보로 나옴(보고에 관측 시각 표지가 없어 “실제 초과”와 “호출자의 미래 주장”을 구분할 수 없음) → S5.6이 escalate → ticket이 `pending`을 떠남 → **S5.4의 stale 판정(`status != 'pending'`)이 아직 SLA 안이던 워커의 유효한 답을 거부** → ⑧이 “TTL보다 느린 유효한 답을 버려 사용자 결과를 잃는 회귀”라며 명시 거부한 바로 그 실패. 악의는 필요 없다(clock drift·운영 플래그·테스트 double 누수·tz 실수). 따라서 S5.6은 다음을 지킨다.
  1. **escalation UoW에 `now` 인자를 두지 않는다.** 시간 원천은 생성자 주입 `clock` 한 지점이다(⑤·S5.2와 같은 계약). `inspect` 시그니처 red로 고정한다.
  2. **`begin_immediate()` 안에서 ticket·Request를 재조회하고, 재조회한 `AwaitingAnswer.handling.due_at`을 자기 `clock()`과 다시 비교한다.** `candidate.due_at`도 후보 목록의 존재 자체도 근거로 쓰지 않는다. 시간 안전성만의 요구가 아니라 **TOCTOU 때문에도 필수**다 — 스캔과 write 사이에 답이 도착해 ticket이 `completed`로 갔을 수 있다.
  3. **감사에는 재도출한 `due_at`과 자기 `now`를 남긴다.** 스캐너가 준 값을 기록하지 않는다(기록된 시각이 판정한 시각과 같아야 감사가 성립한다).
  4. **`DispatchTimeoutScanReport`에 canonical `scanned_at`을 추가한다**(승인). 2가 지켜지면 정확성에는 불필요하지만, 보고가 **자기 기준 시각을 스스로 밝히는 값**이 되어 “감사 불가”가 “감사 가능”으로 바뀐다. 값은 스캔이 실제로 사용한 그 `now`여야 하고(두 번째 clock 읽기 금지) S5 canonical instant 형식이며, **write 근거로 쓸 수 없다**(4는 3을 대체하지 않는다).
  5. **⑨과 모듈 docstring에 “작업 목록 ≠ write 근거”를 명시한다.** 없으면 현재의 자신 있는 문면이 훗날 `now`를 write 경로로 관통시키는 근거로 인용된다 — 그 문면은 내가 썼고, 지역적으로 참인 주장을 무조건적으로 읽히게 쓴 것이 이 결함의 원인이다.
  - **관측이 멈추면 조용히 멈추지 않는다(P2-4 판정).** `reconcile_rows=True`라서 **같은 조직 범위 안의 무관한 손상 1행**이 timeout scan을 지속적으로 `capable=False`로 만들 수 있다(타 조직 row는 org 필터로 opaque하고, catalog/schema 손상은 전역이다). 관측을 fail-closed로 닫는 것 자체는 옳다 — 검증할 수 없는 행을 신뢰해 escalate하는 것이 더 나쁘다. 그러나 S5.6은 **`capable=False`를 “할 일이 없음”과 구분되게 표면화해야 한다** — 리포트에 스캔 capability를 별 필드로 실어, 손상으로 인한 정지가 정상 유휴처럼 보이지 않게 한다(⑫·⑭의 “실패를 정상 관찰처럼 세지 않는다”와 같은 규율이며 새 원칙이 아니다). 새 알림 체계를 만들라는 뜻은 아니다.
- **⑱ one-shot 게이트의 경로 오류도 typed report로 닫는다 (2026-07-26·S5.5 P2-2 판정·S4.6 공통 교정).** `mode=ro` connection을 여는 게이트들이 `sqlite3.Error`만 잡는데, 그 앞의 `Path(...).expanduser().resolve().as_uri()`는 `ValueError`(임베디드 NUL 등)·`RuntimeError`(`~unknownuser` 확장 실패)·`OSError`를 던진다. 이 예외들이 raw로 누수하면 **“capability가 서지 않으면 typed report로 fail-closed한다”는 게이트의 존재 이유 자체가 그 입력에 대해 성립하지 않는다**(호출자가 `capable=False`를 받는 대신 예외로 중단된다). 잡을 집합은 `(ValueError, RuntimeError, OSError, sqlite3.Error)`로 좁히고 `Exception`으로 넓히지 않는다(프로그래밍 오류를 삼키지 않기 위해). **이 결함은 S4.6 `sqlite_durable_linked_reconciliation`에서 상속된 것이므로 두 모듈을 함께 고친다** — 한쪽만 고치면 다음 독자가 나머지 한쪽을 의도로 읽는다. ADR 0066 §3의 `attempt == 1` fence와 같은 성격의 **랜딩된 슬라이스에 대한 명시적 교정**이다.
