...
- **살아있는 지식·SSO·실시간 충돌 = 다음 방향의 핵심(ADR 0017·Phase 7).** ① **SSO 신원 binding**(owner 종속 실재화 — 지금 무비밀번호라 명목뿐, T7.1) ② **git 저장 + 빌더 UI 편집**(owner는 git 몰라도 됨·커밋 스냅샷 실행, T7.2) ③ **지식 신선도·변경 전파**(정책 변경→관련 Precedent·답 자동 재검토/무효화, T7.3) ④ **실시간 충돌 푸시 통지**(지금 pull→push, T7.4). 설계 방향만(세부 슬라이스는 후속).
