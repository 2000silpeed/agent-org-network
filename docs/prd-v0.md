...
| 개인 Owner | 에이전트 빌더 · Owner 처리함 · **owner 클라이언트(Phase 9)** | 자기 에이전트를 깎고, 자기에게 온 후보 합의 + 부재 중 백업이 자기 이름으로 낸 답 검토(승인·정정·무시, ADR 0012)를 처리. **Phase 9: owner OAuth 멀티-LLM 클라이언트로 LLM 초안 검토·수정·전송(HITL on) 또는 자동(off, ADR 0025·0027·T9.7)** |
