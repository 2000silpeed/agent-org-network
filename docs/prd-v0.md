...
| 개인 Owner | Agent 빌더 · Owner 처리함 · **owner 클라이언트(Phase 9)** | 자기 에이전트를 깎고, 자기에게 온 후보 합의 + 부재 중 백업이 자기 이름으로 낸 답 검토(승인·정정·무시, ADR 0012)를 처리. **Phase 9: owner OAuth 멀티-LLM 클라이언트로 LLM 초안 검토·수정·전송(HITL on) 또는 자동(off, ADR 0025·0027·T9.7)** |
| Manager | Manager 큐 | 승인 요청·escalation·합의 실패를 처리 |
| 운영자 | 모니터링 로그 · 상세 보기 · Org 그래프 · **운영 콘솔(Phase 9)** | 전체 그림 + 모든 Q&A의 절차·답 추적. **Phase 9: SSE 실시간 피드(질문 인입·라우팅·답 전송·워커 연결/해제) + POST 명령(세션 종료·HITL 토글·토큰 발급·워커 승인/취소, ADR 0024·0026·T9.2)** |
