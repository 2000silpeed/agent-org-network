...
## S13 — stage-2 하이브리드 리랭크 채택 (ADR 0028 §17-c)

**구조**: embedding 사슬이 오라우팅을 방어하며 풀 수 있는 걸 다 풀고, *그래도 Contested로 남는 잔여에만* LLM 2차(자체 margin)를 투입 — S12에서 확인한 "LLM 결정력 vs 가드 충돌"을 계단식으로 해소. `TwoStageRouter(secondary_assessor, secondary_clear_winner_margin)` 주입·`AssessorChain`·`AON_ASSESSOR=hybrid`(opt-in — 기본 auto는 무지연).

**스윕 판정 (S12 raw 표 재사용·잔여 55 pairs 완전 커버 검증 후 오프라인)**: 2차 margin=0.6·min_confidence=0.3 채택.

| 구성 | top-1 | 오라우팅 | contested | ambiguous |
|---|---|---|---|---|
| embedding 단독 (기준선) | 50.0% | 1.4% | 34.7% | 44.4% |
| **hybrid (m2' 0.6·minc' 0.3)** | **54.2%** | **1.4% (신규 오라우팅 0)** | **30.6%** | **50.0%** |

- 회수 3문항 전부 정답(easy 근로시간·hard 개인정보 열람·ambiguous 연장근로수당). §17-c가 미보장으로 남긴 "잔여 집합 오염도"가 이 config에선 가드를 안 깼다 — LLM이 실제 가를 수 있는 쌍만 회수하고 사람-합의 라벨을 확신 라우팅하지 않음.
- minc' 0.5는 저신뢰 회수분을 버리고 오라우팅만 늘어 전 조합 가드 위반 — 전원 저신뢰에도 2차를 태우는 결정(§17-c)이 실측으로 정당화됨.
- **지연 계약 실측**: 2차 발동 25/72문항 × 평균 2.2후보 × 17.2s ≈ 문항당 ~38s(비잔여 47문항은 0). 동기 라우팅 경로라 opt-in 유지 — 2차 비동기화는 후속 명시 연기.

## 최종 누적 궤적

| 단계 | top-1 | 오라우팅 | contested |
|---|---|---|---|
| 토큰 오버랩 (시작점) | 20.8% | 15.3% | 58.3% |
| + EmbeddingAnnMatcher(τ0.85) | 38.9% | 1.4% | 45.8% |
| + stage-1.5(δ0.03) | 43.1% | 1.4% | 41.7% |
| + stage-2 embedding(m2 0.02) | 50.0% | 1.4% | 34.7% |
| **+ stage-2b 하이브리드 LLM(m2' 0.6)** | **54.2%** | **1.4%** | **30.6%** |
| (참고: e5-large 교체 시 단독) | 54.2% | 1.4% | 37.5% |
