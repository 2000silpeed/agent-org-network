...
> Phase 12 이후 워커는 등록 직후 자기 지정 경계(`AON_KNOWLEDGE_PATHS`, 기본 `okf/<agent_id>/`)의
> 지식을 중앙 Knowledge Store로 동기화하고, WS 연결 상태가 그 담당자의 프레즌스로 잡힌다.
> 카드 등록·오너 변경은 관리 UI(`/admin`), 답 열람·사후 정정은 담당자 감독(`/supervision`) —
> 둘 다 README의 "관리 UI"·"담당자 감독" 절 참고.
