...
def select_author(card: AgentCard) -> OkfAuthor:
    ...
    if flag in _LLM_AUTHOR_ALIASES:
        ...
        print(
            ...
            f"owned_domains={tuple(card.domains)}) — owner Claude 구독 자격 위임·실 추출(게이트 밖)."
        )
        # card.domains를 owned_domains로 주입 — split 프롬프트에 유효 domain 제약을 실어야 LLM이
        # in-scope 개념에 정확한 domain을 골라 admit_okf가 over-claim으로 전량 드롭하지 않는다(치명 갭).
        return LlmAuthor(
            ClaudeCodeTransport(model=model),
            model=model,
            owned_domains=tuple(card.domains),
        )
    raise SystemExit(
