...
from agent_org_network.runtime import AgentRuntime, Answer, ClaudeCodeRuntime
...
def _is_safe_path_component(name: str) -> bool:
    """단일 파일명 컴포넌트(구분자·상대경로·절대경로 0)인지 검증한다(화이트리스트).

    다음을 거부한다:
      - 빈 문자열
      - 예약 stem: "." ".."
      - os.sep("/" on POSIX·"\\" on Windows)·'/'·'\\' 포함 — 구분자가 있으면 다단계 경로
      - 절대경로(os.path.isabs)
      - Path(x).name != x — 상위 경로 성분이 숨어 있음

    순수 파일 stem("refund-policy", "pricing_v2") 등만 통과한다.
    워커가 최종 신뢰 경계이므로 HTTP body 신뢰 불가 입력 전부를 이 게이트로 잠근다.
    """
    if not name:
        return False
    if name in (".", ".."):
...
def main() -> None:
    ...
    # AON_PROVIDER=claude-api면 owner OAuth 인프로세스 SDK 스트리밍으로 교체된다(_select_runtime).
    logic = WorkerLogic(
        owner_id=owner_id,
        cards=cards,
        runtime=_select_runtime(DEMO_OKF_ROOT),
        role=role,
