...
def test_LlmAuthor_분기도_같은_OkfAuthor_포트(monkeypatch: pytest.MonkeyPatch) -> None:
    """치명 갭 교정의 현재 형태 — 유효 domain 제약은 생성자 상태가 아니라 파이프라인이
    split 호출 시 card.domains를 allowed_domains 인자로 넘겨 흐른다(run_authoring_pipeline).

    select_author는 어댑터 선택만 책임진다 — claude-code 분기가 LlmAuthor를 돌려주는지만 단언."""
    monkeypatch.setenv("AON_AUTHOR", "claude-code")
    author = select_author(_card())
    assert isinstance(author, LlmAuthor)
