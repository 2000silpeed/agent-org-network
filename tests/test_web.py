...
from agent_org_network.demo import build_demo, build_demo_ask_org
...
def test_데모_주차장질문은_unowned로_미아되지_않는다():
    reply = build_demo_ask_org(runtime=StubRuntime()).handle(
        "주차장 정기권 어떻게 갱신해요?", _USER
    )
...
def test_serialize_outcome_still_open():
    body = serialize_outcome(StillOpen(case=_sample_case(), pending_owners=("finance_lead",)))
...
def _legacy_concurrence_client() -> tuple[TestClient, str]:
    """legacy Consensus·Precedent HTTP 계약용 request_id=None Case."""
    queue_store = InMemoryManagerQueueStore()
    bundle = build_demo(runtime=StubRuntime(), manager_queue_store=queue_store)
    case = ConflictCase(
        intent="보상",
        question=_CONTESTED_Q,
        candidates=(
            Candidate(agent_id="cs_ops", owner="cs_lead"),
            Candidate(agent_id="finance_ops", owner="finance_lead"),
        ),
        opened_at=datetime(2026, 6, 21, 12, 0, tzinfo=timezone.utc),
        case_id="legacy-web-case",
    )
...
def _open_case_id(client: TestClient) -> str:
    ...
    P17 사용자 표면은 책임 확정 전 답을 금지하므로 Pending을 돌려주고, 같은 DemoBundle의
    request-aware ConflictCase는 처리함에서 계속 조회할 수 있다.
    """
    contested = _post(client, "/ask", {"question": _CONTESTED_Q})
    assert contested.status == 200
    assert contested.body["type"] == "pending"
...
def test_concur_한_표는_still_open():
    client, case_id = _legacy_concurrence_client()
...
def test_legacy_concur는_additive_round와_stance를_무시한다() -> None:
    queue_store = InMemoryManagerQueueStore()
    bundle = build_demo(runtime=StubRuntime(), manager_queue_store=queue_store)
    case = ConflictCase(
        intent="보상",
        ...
        opened_at=datetime(2026, 6, 21, 12, 0, tzinfo=timezone.utc),
        case_id="legacy-additive-fields",
    )
    bundle.case_store.open_case(case)
    with patch("agent_org_network.web.build_demo", return_value=bundle):
        client = TestClient(create_app(runtime=StubRuntime(), manager_queue_store=queue_store))

    first = _post(
        client,
        f"/cases/{case.case_id}/concur",
        {
            "by_owner": "cs_lead",
            "on_agent": "cs_ops",
            "expected_round": 99,
...
def test_concur_비후보_Owner는_403():
    ...
    res = _post(
        client, f"/cases/{case_id}/concur", {"by_owner": "legal_lead", "on_agent": "cs_ops"}
    )
    assert res.status == 403


# ── P17 request_id identity alias 조회(GET /ask/{tracking}) + 노출 불변식 ──────
...
def _ws_demo_app() -> tuple[TestClient, WebSocketDispatcher]:
    """WS sidecar가 있어도 P17 사용자 질문은 중앙/로컬 완성 답 Runtime만 쓰는 데모 앱."""
...
def test_회수_응답에_라우팅_내부값이_새지_않는다():
    ...
    asked = _post(client, "/ask", {"question": "환불 되나요?"})
    request_id: str = asked.body["request_id"]
    after: dict[str, Any] = _get(client, f"/ask/{request_id}").body
...
def test_회수_alias는_request_id이고_WorkTicket을_만들지_않는다():
    client, ws = _ws_demo_app()

    asked = _post(client, "/ask", {"question": "환불 되나요?"})
    request_id: str = asked.body["request_id"]

    assert "tracking" not in asked.body
    assert _get(client, f"/ask/{request_id}").body["request_id"] == request_id
    assert ws.claim("cs_lead") is None


# ── P17 Finalization 사용자 결과 통합 (web 레벨) ─────────────────────
#
# `/ask`가 P17 application 결과를 호환 JSON으로 투영하고 세션 내부값을 숨기는지 검증한다.


def test_web_ask_P17_Answered_직렬화에_세션_내부값이_없다():
    """P17 Answered 호환 응답에 세션 내부값이 없다.
...
def test_web_ask_연속_요청이_모두_정상_응답을_반환한다():
    """같은 앱 인스턴스에서 두 P17 Request가 각각 정상 종결된다."""
    client = _client()
...
# ── P17 익명 requester cookie와 legacy SessionStore 비사용 ──────────────
...
def test_쿠키없는_첫요청은_legacy_store에_세션을_만들지_않는다():
    """P17 Finalization과 legacy SessionStore를 이중 기록하지 않는다."""
    store = InMemorySessionStore()
    client = _cookie_client(store)
    ...
    assert store.active_for_user(uid) is None


def test_같은_쿠키_재요청도_legacy_store를_쓰지_않는다():
    store = InMemorySessionStore()
    client = _cookie_client(store)
    ...
    http: Any = client
    res2 = cast(
        Response, http.post("/ask", json={"question": "계약 기간은?"}, cookies={"aon_uid": uid})
    )
...
def test_다른_쿠키_브라우저도_legacy_store에_세션을_만들지_않는다():
    store = InMemorySessionStore()
    client = _cookie_client(store)
    ...
    res_a = cast(
        Response,
        http.post(
            "/ask", json={"question": "계약서 검토해줄 수 있어?"}, cookies={"aon_uid": uid_a}
        ),
    )
    res_b = cast(
        Response, http.post("/ask", json={"question": "환불 되나요?"}, cookies={"aon_uid": uid_b})
    )

    assert res_a.status_code == 200
    assert res_b.status_code == 200

    assert store.active_for_user(uid_a) is None
    assert store.active_for_user(uid_b) is None
...
def test_ask_stream_accepted_token_done_순서로_흐른다():
    ...
    assert "".join(token_texts) == "환불은 7일 이내 가능합니다."


def test_ask_stream_done은_담당과_출처를_싣는다():
    client = _stream_app()
    http: Any = client
    res = cast(Response, http.post("/ask/stream", json={"question": "환불 되나요?"}))
    ...
    assert done["answered_by"] == "cs_lead"
    assert done["agent_id"] == "cs_ops"
    assert "mode" in done
    assert "sources" in done


def test_ask_stream_token에_내부값이_새지_않는다():
    # token은 request 상관키와 텍스트만 싣고 책임·정책 내부값은 싣지 않는다.
    client = _stream_app()
    http: Any = client
    res = cast(Response, http.post("/ask/stream", json={"question": "환불 되나요?"}))
    ...
    for name, payload in frames:
        if name == "token":
            assert set(payload.keys()) == {"request_id", "text"}
...
def test_ask_stream_런타임_예외는_interrupted_프레임으로_투영된다():
    # 노출 불변식: 런타임 예외·스택은 숨기고 저장된 Request ID의 중립 interrupted만 보낸다.
    ...
    assert res.status_code == 200
    frames = _parse_sse(res.text)
    interrupted = [payload for name, payload in frames if name == "interrupted"]
